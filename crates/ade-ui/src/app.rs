//! Root view: full ADE layout shell.
    ...
    fn flat_models(&self) -> Vec<(String, String)> {
        self.store
            ...
            .flat_map(|p| {
                p.models
                    .iter()
                    .map(move |m| (p.provider_id.clone(), m.0.clone()))
            })
            .collect()
    }
...
impl AdeApp {
    fn render_top_bar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let (dot, status_text) = match &self.store.conn {
            ...
            ConnState::Disconnected(_) => (bad_color(), "disconnected"),
        };
...
impl AdeApp {
    fn session_rows(&self) -> Vec<SessionRow> {
        let mut ids: Vec<_> = self.store.sessions.keys().cloned().collect();
        ids.sort_by_key(|id| {
            std::cmp::Reverse(
                self.store
                    .sessions
        ...
        ids.into_iter()
            .filter_map(|id| {
                let s = self.store.sessions.get(&id)?;
                let dot = self.store.statuses.get(&id).and_then(|st| match st {
                ...
                Some(SessionRow {
                    ...
                    active: self.store.active_session.as_deref() == Some(id.as_str()),
                    status_dot: dot,
                    id,
                })
            })
    ...
    fn render_sidebar(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let new_btn = cx.listener(|this, _: &ClickEvent, _, _| {
        ...
        div()
            ...
            .child(
                uniform_list("sessions", rows.len(), {
                    let rows = rows.clone();
                    move |_range, _window, _cx| rows.iter().map(session_row_element).collect()
                })
                .flex_1()
                .min_h_0(),
            )
...
fn session_row_element(row: &SessionRow) -> AnyElement {
    let bg: Hsla = if row.active {
    ...
    div()
        ...
        .children(
            row.status_dot
                .map(|c| Icon::new(IconName::CircleCheck).xsmall().text_color(c)),
        )
        .into_any_element()
}
...
impl AdeApp {
    ...
    fn render_composer(&self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
...
impl AdeApp {
    fn render_right_panel(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let body: AnyElement = match self.right_tab {
            ...
            RightTab::Diff => diff_view(&self.store, cx),
        };
...
fn context_view(store: &Store, cx: &mut Context<AdeApp>) -> AnyElement {
    ...
    for s in &view.sections {
        let color = match s.kind {
            ...
            SectionKind::Tools | SectionKind::Other => muted_color(),
        };
        col = col.child(
            div()
                .rounded_sm()
...
fn diff_view(store: &Store, cx: &mut Context<AdeApp>) -> AnyElement {
    if store.diffs.is_empty() {
        return v_center("No diff fetched yet — press ⇄ next to a session.");
    }
    let fg = cx.theme().foreground;
